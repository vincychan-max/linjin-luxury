...
export default function ProductListClient({ 
  ...
  // 3. 【核心修复】：页面挂载时只同步一次数据库状态
  // 去掉依赖项中的 fetchWishlistIds，防止无限循环
  useEffect(() => {
    fetchWishlistIds();
  }, []); 
  ...
  const finalProducts = useMemo(() => {
    ...
    let list = [...initialProducts].filter((p: any) => 
  ...
  return (
    <div className="min-h-screen bg-white text-zinc-900 font-light">
      ...
      <div className="relative h-[65vh] w-full flex items-center justify-center bg-zinc-900 overflow-hidden">
        {collectionBackgroundImageUrl && (
          <img src={collectionBackgroundImageUrl} className="absolute inset-0 w-full h-full object-cover opacity-60" alt="" />
        )}
      ...
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-zinc-100 flex items-center justify-center">
        ...
        <div ref={scrollRef} className="flex gap-10 py-7 overflow-x-auto no-scrollbar scroll-smooth px-4 snap-x snap-mandatory"> {/* 新增：snap-x 手机滑动卡位 */}
          {subCategoriesList?.map((item: any) => (
            <Link key={item.slug} href={`/${gender}/${category}/${item.slug}`} className={`text-[10px] uppercase tracking-[0.2em] whitespace-nowrap transition-all snap-center ${subCategory === item.slug ? 'text-black font-bold border-b border-black pb-1' : 'text-zinc-400 hover:text-black'}`}> {/* 新增：snap-center */}
              {item.name}
            </Link>
          ))}
        </div>
      ...
      {/* 搜索控制区 */}
      <div className="max-w-[1536px] mx-auto px-12 py-10 flex flex-wrap md:flex-nowrap justify-between items-center border-b border-zinc-50 gap-6"> {/* 新增：md:flex-nowrap 大屏一行 */}
      ...
      <main className="max-w-[1536px] mx-auto px-12 py-20" onClick={() => setIsFilterOpen(false)}>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-10 gap-y-24"> {/* 优化：手机1列，sm:2列，lg:4列 */}
          {finalProducts.map((p: any) => {
            const currentIdx = productImageIndexes[p.id] || 0;
            // 4. 使用 Store 里的 ID 判断状态
              ...
              <div key={p.id} className="group relative flex flex-col">
                <div className="relative aspect-[3/4] bg-[#fcfcfc] overflow-hidden">
                  ...
                  <button 
                    ...
                    className={`absolute top-5 right-5 z-40 p-3 transition-all duration-500 hover:scale-110 ${isLiked ? 'opacity-100' : 'opacity-0 group-hover:opacity-100 group-focus-within:opacity-100'}`} // 优化：p-3 增大触摸区；focus-within 支持 tap
                  >
                    <Heart 
                  ...
                  {p.images.length > 1 && (
                    <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 flex justify-between px-3 z-30 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity"> {/* 优化：focus-within 支持 tap */}
